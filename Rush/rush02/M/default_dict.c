...
void	defalut_dict(char **key, char **val)
{
	int		i;

	i = 0;
	key = malloc(sizeof(char *) * 32);
	val = malloc(sizeof(char *) * 32);
	while (i < 32)
	{
		key[i] = malloc(sizeof(char) * 11);
		val[i] = malloc(sizeof(char) * 11);
		i++;
	}
	fill_0_to_19_key(key);
	fill_20_to_bil_key(key);
	fill_0_to_19_val(val);
	fill_20_to_bil_val(val);
}
